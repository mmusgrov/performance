...
/**
 * JMH Benchmark for JGroupsSlots with WAL + fsync enabled (disaster recovery mode).
 *
 * <p>Configuration:
 * <ul>
 *   <li>WAL enabled: true</li>
 *   <li>WAL fsync writes: true (maximum durability)</li>
 *   <li>WAL fsync deletes: true</li>
 * </ul>
 *
 * <p>This provides crash + power failure safety (disaster recovery).
 */
@State(Scope.Benchmark)
public class JGroupsSlotsBenchmark_WAL_Fsync extends JTAStoreBase {
    static final int THREADS = 240;
    static final String BM_CLASS_NAME = JGroupsSlotsBenchmark_WAL_Fsync.class.getSimpleName();
    ...
    public static void setup() throws CoreEnvironmentBeanException {
        ...
        configBean.setNodeAddress("benchmark-node-wal-fsync");
        configBean.setGroupName("jgroups-slots-wal-fsync-" + System.currentTimeMillis());
        configBean.setReplicationCount((short) -1);

        // WAL configuration - DISASTER RECOVERY MODE
        configBean.setWalEnabled(true);       // Enable WAL
        configBean.setWalSyncWrites(true);    // fsync on writes (crash + power failure safe)
        configBean.setWalSyncDeletes(true);   // fsync on deletes
